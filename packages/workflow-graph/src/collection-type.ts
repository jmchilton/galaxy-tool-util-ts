...
export class CollectionTypeDescription implements CollectionTypeDescriptor {
  ...
  /**
   * Symmetric sibling-matching check: do `this` and `other` match such
   * that they could drive a common map-over over sibling inputs of a
   * single tool? Used for sibling map-over state checks where neither
   * side is the declared input slot.
   */
  ...
  canMapOver(other: CollectionTypeDescriptor) {
    ...
    if (
      other.collectionType.startsWith("sample_sheet") &&
      !this.collectionType.startsWith("sample_sheet")
    ) {
      return false;
    }
